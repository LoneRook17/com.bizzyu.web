...
// Shared support-chat UI, used by both audiences:
//   • student  — /support-chat page inside the iOS app WebView (fullscreen)
//   • business — the dashboard help bubble (panel)
// The streaming/send logic is identical for both; the only difference is auth
// (students pass a Bearer token; businesses ride the session cookie) and the
// outer chrome (variant). Device-local history is layered on for both.

type Msg = { role: "user" | "assistant"; content: string }

export interface SupportChatProps {
  audience: "student" | "business"
  /** Stable per-user identifier for the history key — hashed, never stored raw. */
  historySeed: string
  /** Student Sanctum token; omitted for business (rides the session cookie). */
  token?: string
  welcome: string
  suggestions: string[]
  placeholder?: string
  headerLabel?: string
  headerNote?: string
  /** fullscreen = the WebView page; panel = the dashboard bubble. */
  variant?: "fullscreen" | "panel"
  /** When provided (panel), renders a close button in the header. */
  onClose?: () => void
}
...
export default function SupportChat({
  ...
  headerLabel = "Bizzy Assistant",
  headerNote = "· usually replies instantly",
  variant = "fullscreen",
  onClose,
}: SupportChatProps) {
  ...
  }, [audience, historySeed])

  // Persist on every message change (only after the initial restore, so we
  // never clobber stored history with the empty starting state).
  useEffect(() => {
  ...
  // On restore, jump to the bottom immediately (no smooth animation).
  useEffect(() => {
    if (storageKey && messages.length) bottomRef.current?.scrollIntoView({ block: "end" })
  ...
  async function send(text: string) {
    ...
    try {
      ...
      if (!res.ok || !res.body) {
        ...
        else setFatal("Something went wrong on our end. Try again, or email support@bizzyu.com.")
