...
/** v2-styled user search with debounced lookup against /business/team/search-users. */
export function UserSearchInput({
  ...
  const handleChange = (val: string) => {
    ...
    debounceRef.current = setTimeout(async () => {
      setLoading(true)
      try {
        const data = await apiClient.get<UserResult[]>(`/business/team/search-users?q=${encodeURIComponent(val.trim())}`)
        setResults(data)
