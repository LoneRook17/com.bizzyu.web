...
import type { EventAnalytics, PerScannerRow } from "@/lib/business/types"
...
export function EventAnalyticsView({ data, perScanner }: { data: EventAnalytics; perScanner?: PerScannerRow[] | null }) {
    ...
    <div className="flex flex-col gap-5">
      ...
      <Card>
        ...
        <CardContent className="pt-0">
          ...
          <p className="mt-1 text-[13px] text-neutral-500 dark:text-neutral-400">{hasTips(data.revenue) ? REVENUE_CAPTION_WITH_TIPS : "Your take-home: matches Stripe payout."}</p>
        </CardContent>
      </Card>
      ...
      {eventTipsVisible(data, perScanner) && (
        <Card>
          ...
          <CardContent className="pt-0">
            <p className="text-2xl font-semibold tracking-tight text-neutral-900 dark:text-neutral-100">{usd(eventTipsAmount(data, perScanner))}</p>
      ...
      <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
        ...
        <Card>
          ...
          <CardContent className="pt-0">
            <p className="mb-2 text-2xl font-semibold tracking-tight text-neutral-900 dark:text-neutral-100">{checkPct.toFixed(1)}%</p>
            <div className="h-3 w-full overflow-hidden rounded-full bg-neutral-100 dark:bg-neutral-800">
              <div className="h-full rounded-full bg-[#05EB54]" style={{ width: `${Math.min(checkPct, 100)}%` }} />
            </div>
            <p className="mt-2 text-[13px] text-neutral-500 dark:text-neutral-400">{data.checkIn.scanned} scanned / {data.checkIn.total} total</p>
          </CardContent>
        </Card>
      </div>
