...
// total_revenue is the new canonical sort key. Older service deploys only
// emit `revenue` (which is itself the alias the new server writes); fall back
// so an old API + new client still sorts sensibly.
...
export default function DoorPerformanceCard({
  ...
  if (rows.length === 0) {
    return (
      <div className="rounded-xl border border-gray-200 bg-white p-5">
        ...
        <p className="text-sm text-gray-400">No door activity recorded yet.</p>
      </div>
    )
  }
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-5">
      <h3 className="text-sm font-semibold text-ink">Door Performance</h3>
      <p className="text-xs text-gray-500 mt-0.5 mb-3">{DOOR_TIPS_HINT}</p>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-100">
              ...
              <th className={`${cellHeader} text-right`} onClick={() => clickHeader("tips")}>{DOOR_TIPS_COLUMN_TITLE}{arrow("tips")}</th>
          ...
          <tbody>
            {sorted.map((r, idx) => (
              <tr key={`${r.staff_user_id ?? "null"}-${r.scanner_label ?? "null"}-${idx}`} className="border-b border-gray-50 last:border-0">
                ...
                <td className="py-2 text-right text-gray-600">{formatCurrency(scannerTips(r))}</td>
