...
export default function V2EventDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // WC FLAW 3 — a Weekly Cover night that still follows its program must not
  // take price edits on the event surface. Either the write stamps
  // series_customized_at (restamp then skips the night forever), or the next
  // weekday-global restamp silently overwrites the new price. The night
  // editor's override is correct in both directions, so the pencil routes
  // there. Null for named events and for nights already customized.
