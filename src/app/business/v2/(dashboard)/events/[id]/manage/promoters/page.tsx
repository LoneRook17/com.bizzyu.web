          ...
          <Card>
            <CardContent>
              <div className="mb-2 flex items-center gap-2">
                <Megaphone className="size-4 text-[#079455]" />
                <h3 className="text-sm font-semibold text-neutral-600">Going to promoters</h3>
              </div>
              <p className="text-2xl font-semibold tracking-tight text-neutral-900">{money(totalCommissionCents)}</p>
                  ...
                  <div key={p.tracking_link_id} className="flex flex-col gap-3 p-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="truncate text-sm font-semibold text-neutral-900">{p.full_name ?? "Unknown"}</p>
                        <p className="truncate font-mono text-xs text-neutral-500">{p.code}</p>
                      </div>
                    ...
                    <div className="flex items-center gap-2 rounded-lg bg-neutral-50 px-3 py-2">
                      <p className="min-w-0 flex-1 truncate font-mono text-xs text-neutral-600" title={p.share_url}>{p.share_url}</p>
                      <button
                        onClick={() => copyShareUrl(p)}
                        className="inline-flex shrink-0 items-center gap-1 text-xs font-semibold text-[#079455] hover:underline"
                      >
...
function Stat({ value, label, accent }: { value: string; label: string; accent?: boolean }) {
  return (
    <div className="text-center">
      <p className={accent ? "font-semibold text-[#079455]" : "font-semibold text-neutral-700"}>{value}</p>
