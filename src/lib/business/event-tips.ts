...
// deploys that predate the field.
//
...
export function tipsVisible(data: AnalyticsWithTipsFlags): boolean {
  ...
  return data.tips_visible === true || data.tipping_enabled === true || eventTips(data.revenue) > 0
}
...
export function doorTipsVisible(
  ...
  analytics: AnalyticsWithTipsFlags,
  rows: readonly ({ tips?: unknown } | null | undefined)[] | null | undefined,
): boolean {
  if (perScannerFlag === true || tipsVisible(analytics)) return true
  return (rows ?? []).some((r) => scannerTips(r) > 0)
}
